...
@section('content')
    ...
    <section class="wrapper-bottom-sec">
        <div class="p-30">
            <h2 class="page-title">{{language_data('Employee Summary')}}</h2>
        </div>
                            ...
                            <div class="form-block">
                                ...
                                <div class="form-group">
                                    ...
                                    @if($employee->status=='active')
                                        <span class="bold text-success text-underline">{{language_data($employee->status)}}</span>
                                    @else
                                        <span class="bold text-danger text-underline">{{language_data($employee->status)}}</span>
                                    @endif
                                </div>

                                <div class="form-group">
                                    <label>{{language_data('Gender')}}:</label>
                                    <span>{{language_data($employee->gender)}}</span>
                                </div>

                                <div class="form-group">
                                    <label>{{language_data('Payment Type')}}:</label>
                                    <span>{{language_data($employee->payment_type)}}</span>
                                </div>
                        ...
                        <div class="panel-body">
                            <table class="table data-table table-hover table-ultra-responsive">
                                ...
                                @foreach($provident_fund as $pf)
                                    <tr>
                                        <td data-label="{{language_data('Provident Fund Type')}}">
                                            <p>{{language_data($pf->provident_fund_type)}}</p></td>
                                        ...
                                        @if($pf->status=='Paid')
                                            <td data-label="{{language_data('Status')}}">
                                                <p class="bold text-success text-underline">{{language_data('Paid')}}</p></td>
                                        @else
                                            <td data-label="{{language_data('Status')}}">
                                                <p class="bold text-warning text-underline">{{language_data('Unpaid')}}</p></td>
                                        @endif
                                    </tr>
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            <table class="table data-table table-hover table-ultra-responsive">
                                ...
                                @foreach($loan as $l)
                                    <tr>
                                        <td data-label="{{language_data('Title')}}">{{$l->title}}</td>
                                        <td data-label="{{language_data('Date')}}"><p>{{get_date_format($l->loan_date)}}</p></td>
                                        <td data-label="{{language_data('Repayment Start Date')}}">{{get_date_format($l->repayment_start_date)}}</td>
                                        <td data-label="{{language_data('Amount')}}">{{app_config('CurrencyCode')}} {{$l->amount}}</td>
                                        <td data-label="{{language_data('Remaining Amount')}}">{{app_config('CurrencyCode')}} {{$l->remaining_amount}}</td>
                                        @if($l->status=='ongoing')
                                            <td data-label="{{language_data('Status')}}">
                                                <p class="bold text-success text-underline">{{language_data('Ongoing')}}</p></td>
                                        ...
                                        @else
                                            <td data-label="{{language_data('Status')}}">
                                                <p class="bold text-complete">{{language_data('Completed')}}</p>
                                            </td>
                                        @endif
                                    </tr>
