                            ...
                            <div class="panel-body p-none">
                                <table class="table data-table table-hover table-ultra-responsive">
                                    ...
                                    @foreach($payroll as $d)
                                        <tr>
                                            <td data-label="{{language_data('Code')}}"><a href="{{url
                                            ('employees/view/'.$d->id)}}">{{$d->employee_code}}</a>
                                            </td>
                                            <td data-label="{{language_data('Name')}}">{{$d->fname}} {{$d->lname}}</td>
                                            <td data-label="{{language_data('Department')}}">{{$d->department_name->department}}</td>
                                            <td data-label="{{language_data('Designation')}}">{{$d->designation_name->designation}}</td>
                                            <td data-label="{{language_data('Payment Type')}}">{{language_data($d->payment_type)}}</td>
                                            <td>
                                                @if($d->payment_type=='Hourly' AND $d->working_hourly_rate=='0')
                                                    <p class="bold text-danger text-underline">{{language_data('Set Working Rate')}}</p>
                                                @elseif($d->payment_type=='Monthly' AND $d->basic_salary=='0.00')
                                                    <p class="bold text-danger text-underline">{{language_data('Set Working Rate')}}</p>
                                                @elseif(\App\Attendance::where('emp_id',$d->id)->first())
                                                    <a href="#" id="{{$d->id}}" class="btn btn-success makePayment"><i class="fa fa-credit-card"></i> {{language_data('Make Payment')}}
                                                    </a>
                                                @else
                                                    <p class="bold text-danger text-underline">{{language_data('No working hour')}}</p>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
