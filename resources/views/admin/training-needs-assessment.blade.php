                ...
                <div class="col-lg-12">
                    <div class="panel">
                        <div class="panel-heading">
                            <button class="btn btn-success pull-right" data-toggle="modal" data-target="#add-new-training">
                                <i class="fa fa-plus"></i> {{language_data('Add')}} {{language_data('Training Needs Assessment')}}
                            </button>
                            <br>
                        </div>
                        <div class="panel-body p-none">
                            <table class="table data-table table-hover table-ultra-responsive">
                                ...
                                @foreach($tnassessment as $et)
                                    <tr>
                                        <td data-label="{{language_data('Department')}}"><p>{{$et->department_name->department}}</p></td>
                                        <td data-label="{{language_data('Training Type')}}"><p>{{language_data($et->training_type)}}</p></td>
                                        ...
                                        <td data-label="{{language_data('Status')}}">
                                            @if($et->status=='pending')
                                                <span class="bold text-warning text-underline">{{language_data('Pending')}}</span>
                                            @elseif($et->status=='approved')
                                                <span class="bold text-success text-underline">{{language_data('Approved')}}</span>
                                            @elseif($et->status=='completed')
                                                <span class="bold text-info text-underline">{{language_data('Completed')}}</span>
                                            @else
                                                <span class="bold text-danger text-underline">{{language_data('Rejected')}}</span>
                                            @endif
                                        </td>
                                        <td class="text-right">
                                            <a class="btn btn-success" href="{{url('training/view-training-needs-assessment/'.$et->id)}}"><i class="fa fa-edit"></i> {{language_data('Edit')}}
                                            </a>
                                            <a href="#" class="btn btn-danger cdelete" id="{{$et->id}}"><i class="fa fa-trash"></i> {{language_data('Delete')}}
                                            </a>
                                        </td>
                                    </tr>
