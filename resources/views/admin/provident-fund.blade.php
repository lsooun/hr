                ...
                <div class="col-lg-12">
                    <div class="panel">
                        <div class="panel-heading">
                            <button class="btn btn-success pull-right" data-toggle="modal" data-target="#add-provident-fund">
                                <i class="fa fa-plus"></i> {{language_data('Add')}} {{language_data('Provident Fund')}}</button>
                            <br>
                        </div>
                        <div class="panel-body p-none">
                            <table class="table data-table table-hover table-ultra-responsive">
                                ...
                                @foreach($pfund as $pf)
                                <tr>
                                    <td data-label="{{language_data('Employee Name')}}"><a href="{{url('employees/view/'.$pf->emp_id)}}">{{$pf->employee_info->fname}} {{$pf->employee_info->lname}}</a> </td>
                                    <td data-label="{{language_data('Provident Fund Type')}}"><p>{{language_data($pf->provident_fund_type)}}</p></td>
                                    <td data-label="{{language_data('Employee Share')}}">
                                        <p>
                                    ...
                                    <td data-label="{{language_data('Organization Share')}}">
                                        <p>
                                            @if($pf->provident_fund_type=='Fixed Amount')
                                    ...
                                    @if($pf->status=='Paid')
                                    <td data-label="{{language_data('Status')}}"><p class="bold text-success text-underline">{{language_data('Paid')
                                    }}</p></td>
                                    @else
                                    <td data-label="{{language_data('Status')}}"><p class="bold text-warning text-underline">{{language_data('Unpaid')}}</p></td>
                                    @endif
                                    <td>
                                    @if($pf->status=='Paid')
                                        <a class="btn btn-complete" style="margin-bottom: 5px" href="{{url('provident-fund/view-payslip/'.$pf->id)}}" >
                                            <i class="fa fa-print"></i> {{language_data('Payslip')}}</a>
                                    @else
                                        @if($pf->total!=0)
                                            <a class="btn btn-complete" style="margin-bottom: 5px" href="#" data-toggle="modal"
                                               data-target=".modal_make_payment_pf_{{$pf->id}}">
                                                <i class="fa fa-edit"></i> {{language_data('Make Payment')}}</a>
                                            @include('admin.modal-make-provident-fund-payment')
                                        @endif
                                    @endif

                                        <a class="btn btn-success" style="margin-bottom: 5px" href="{{url('provident-fund/view-details/'.$pf->id)}}" >
                                            <i class="fa fa-edit"></i> {{language_data('Edit')}}</a>
                                        <a class="btn btn-danger cdelete" style="margin-bottom: 5px" href="#" id="{{$pf->id}}">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="form-group m-none">
                                    ...
                                    <select name="fund_type" class="form-control selectpicker fundType">
                                        <option value="Fixed Amount" selected>{{language_data('Fixed Amount')}}</option>
                                        <option value="Percentage of Basic Salary">
                                            {{language_data('Percentage of Basic Salary')}}
                                        </option>
                                    </select>
                                </div>
