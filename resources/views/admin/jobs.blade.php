                    ...
                    <div class="panel">
                        <div class="panel-heading">
                            <button class="btn btn-success pull-right" data-toggle="modal"
                                    data-target="#add-new-job">
                                <i class="fa fa-plus"></i> {{language_data('Add New Job')}}
                            </button>
                            <br>
                        </div>
                        <div class="panel-body p-none">
                            <table class="table data-table table-hover table-ultra-responsive">
                                ...
                                @foreach($jobs as $d)
                                    <tr>
                                        <td data-label="{{language_data('Position')}}"><p>{{$d->position_name->designation}}</p></td>
                                        <td data-label="{{language_data('Posted Date')}}"><p>{{get_date_format($d->post_date)}}</p></td>
                                        <td data-label="{{language_data('Apply Last Date')}}"><p>{{get_date_format($d->apply_date)}}</p></td>
                                        <td data-label="{{language_data('Close Date')}}"><p>{{get_date_format($d->close_date)}}</p></td>
                                        @if($d->status=='opening')
                                            <td data-label="{{language_data('Status')}}">
                                                <p class="bold text-success text-underline">{{language_data('Open')}}</p></td>
                                        @elseif($d->status=='drafted')
                                            <td data-label="{{language_data('Status')}}">
                                                <p class="bold text-warning text-underline">{{language_data('Drafted')}}</p></td>
                                        @else
                                            <td data-label="{{language_data('Status')}}">
                                                <p class="bold text-danger text-underline">{{language_data('Closed')}}</p></td>
                                        @endif
                                        <td>
                                            <a class="btn btn-primary" style="margin-bottom: 5px" href="{{url('jobs/view-applicant/'.$d->id)}}">
                                                <i class="fa fa-list"></i> {{language_data('Applicants')}}
                                            </a>
                                            <a class="btn btn-success" style="margin-bottom: 5px" href="{{url('jobs/edit/'.$d->id)}}">
                                                <i class="fa fa-edit"></i> {{language_data('Edit')}}
                                            </a>
                                            <a class="btn btn-danger cdelete" style="margin-bottom: 5px" href="#" id="{{$d->id}}">
                        ...
                        <form class="form-some-up" role="form" method="post" action="{{url('jobs/post-new-job')}}">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <span class="help">{{language_data('e.g.')}} "软件工程师"</span>
                                ...
                                <div class="form-group">
                                    ...
                                    <span class="help">{{language_data('e.g.')}} "2"</span>
