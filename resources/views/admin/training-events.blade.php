                ...
                <div class="col-lg-12">
                    <div class="panel">
                        <div class="panel-heading">
                            <button class="btn btn-success pull-right" data-toggle="modal" data-target="#add-new-event">
                                <i class="fa fa-plus"></i> {{language_data('Add New Event')}}</button>
                            <br>
                        </div>
                        <div class="panel-body p-none">
                            <table class="table data-table table-hover table-ultra-responsive">
                                ...
                                <tr>
                                    ...
                                    <th style="width: 15%;">{{language_data('Training Subject')}}</th>
                                ...
                                @foreach($training_event as $te)
                                    <tr>
                                        <td data-label="{{language_data('Training Type')}}"><p>{{language_data($te->training_type)}}</p></td>
                                        <td data-label="{{language_data('Training Subject')}}">
                                            <p>{{language_data($te->training_subject)}}</p></td>
                                        <td data-label="{{language_data('Title')}}"><p>{{$te->title}}</p></td>
                                        <td data-label="{{language_data('Training From')}}"><p>{{get_date_format($te->training_from)}}</p>
                                        </td>
                                        <td data-label="{{language_data('Training To')}}"><p>{{get_date_format($te->training_to)}}</p></td>

                                        <td data-label="{{language_data('Status')}}">
                                            @if($te->status=='upcoming')
                                                <span class="bold text-info text-underline">{{language_data('Upcoming')}}</span>
                                            @else
                                                <span class="bold text-success text-underline">{{language_data('Completed')}}</span>
                                            @endif
                                        </td>

                                        <td class="text-right">
                                            <a class="btn btn-success" href="{{url('training/view-training-events/'.$te->id)}}"><i class="fa fa-edit"></i> {{language_data('Edit')}}
                                            </a>
                                            <a href="#" class="btn btn-danger cdelete" id="{{$te->id}}"><i class="fa fa-trash"></i> {{language_data('Delete')}}
                                            </a>
                                        </td>
                                    </tr>
                                @endforeach
                    ...
                    <div class="modal-content">
                        ...
                        <form class="form-some-up" role="form" method="post" action="{{url('training/post-new-training-event')}}">
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    <label>{{language_data('Training Subject')}}</label>
