...
class PayrollController extends Controller {
  ...
  public function getDesignation(Request $request) {
    ...
    if ($dep_id) {
      echo '<option value="0">' . language_data('Select Designation') . '</option>';
  ...
  public function postCustomSearch(Request $request) {
    ...
    $v->setAttributeNames($niceNames);
    if ($v->fails()) {
      return redirect('payroll/make-payment')->withErrors($v->errors());
    }
    ...
    $text_date = date('Y年n月', strtotime($date));
  ...
  public function postNewLoan(Request $request) {
    ...
    $v = \Validator::make($request->all(), [
      'emp_name' => 'required', 'title' => 'required', 'loan_date' => 'required', 'loan_amount' => 'required',
      'payslip' => 'required', 'repayment_amount' => 'required', 'repayment_start_date' => 'required'
    ]);
    $niceNames = array(
      'emp_name' => '员工姓名',
      'title' => '标题',
      ...
      'loan_amount' => '借款金额',
      'payslip' => '从工资中扣减',
      'repayment_amount' => '偿还金额',
      'repayment_start_date' => '开始日期'
    );
    $v->setAttributeNames($niceNames);
    if ($v->fails()) {
